...
#janelas
#janela do oiii
min_oiii = cen_hbeta - 100
max_oiii = cen_oiii1 + 100

#janela do h alpha
min_halpha = cen_nii2 - 100
max_halpha = cen_nii1 + 100

#janela do oii
min_oii = cen_oii1 - 100
max_oii = cen_oii2 + 100
...
lam_fin = np.append(lam_fin1, lam3)
cube_fin = np.append(cube_fin1, cube3)

def gauss(x, a, b, c, d, e, f, g, h, i, j, k):
    ...
    w_oiii1 = (x - d)/e
    alpha_oiii1 = (1./math.sqrt(2.*math.pi))*np.exp((-w_oiii1**2.)/2.)*f
    
    cenoiii2 = lam_oiii2 + (lam_oiii2/lam_oiii1)*(d - lam_oiii1)
    sigoiii2 = (lam_oiii2/lam_oiii1)*e
    ampoiii2 = f/3.
    w_oiii2 = (x - cenoiii2)/sigoiii2
    alpha_oiii2 = (1./math.sqrt(2.*math.pi))*np.exp((-w_oiii2**2.)/2.)*ampoiii2
    
    cenhbeta = lam_hbeta + (lam_hbeta/lam_oiii1)*(d - lam_oiii1)
    sighbeta = (lam_hbeta/lam_oiii1)*e
    w_hbeta = (x - cenhbeta)/sighbeta
    alpha_hbeta = (1./math.sqrt(2.*math.pi))*np.exp((-w_hbeta**2.)/2.)*g
    
    cenhalpha = lam_halpha + (lam_halpha/lam_oiii1)*(d - lam_oiii1)
    sighalpha = (lam_halpha/lam_oiii1)*e
    w_halpha = (x - cenhalpha)/sighalpha
    alpha_halpha = (1./math.sqrt(2.*math.pi))*np.exp((-w_halpha**2.)/2.)*h
    
    cennii1 = lam_nii1 + (lam_nii1/lam_oiii1)*(d - lam_oiii1)
    signii1 = (lam_nii1/lam_oiii1)*e
    w_nii1 = (x - cennii1)/signii1
    alpha_nii1 = (1./math.sqrt(2.*math.pi))*np.exp((-w_nii1**2.)/2.)*i
    
    cennii2 = lam_nii2 + (lam_nii2/lam_oiii1)*(d - lam_oiii1)
    signii2 = (lam_nii2/lam_oiii1)*e
    ampnii2 = i/3.
    w_nii2 = (x - cennii2)/signii2
    alpha_nii2 = (1./math.sqrt(2.*math.pi))*np.exp((-w_nii2**2.)/2.)*ampnii2
    
    cenoii1 = lam_oii1 + (lam_oii1/lam_oiii1)*(d - lam_oiii1)
    sigoii1 = (lam_oii1/lam_oiii1)*e
    w_oii1 = (x - cenoii1)/sigoii1
    alpha_oii1 = (1./math.sqrt(2.*math.pi))*np.exp((-w_oii1**2.)/2.)*j
    
    cenoii2 = lam_oii2 + (lam_oii2/lam_oiii1)*(d - lam_oiii1)
    sigoii2 = (lam_oii2/lam_oiii1)*e
    w_oii2 = (x - cenoii2)/sigoii2
    alpha_oii2 = (1./math.sqrt(2.*math.pi))*np.exp((-w_oii2**2.)/2.)*k
    
    return (r1 + r2 + r3 + (alpha_oiii1/e) + (alpha_oiii2/sigoiii2) + (alpha_hbeta/sighbeta)
+ (alpha_halpha/sighalpha) + (alpha_nii1/signii1) + (alpha_nii2/signii2) + (alpha_oii1/sigoii1) 
+ (alpha_oii2/sigoii2))
    
guess = np.array([10, 10, 10, cen_oiii1, 1, 1, 1, 1, 1, 1, 1])
...
y = (a1 + amp_oiii1*(1./math.sqrt(2.*math.pi))*np.exp((-((x - center_oiii1)/sig_oiii1)**2.)/2.)/sig_oiii1
     ...
     + amp_hbeta*(1./math.sqrt(2.*math.pi))*np.exp((-((x - center_hbeta)/sigma_hbeta)**2.)/2.)/sigma_hbeta)
plt.plot(x,y, color = 'red')

plt.subplot(3,1,2)
plt.plot(lam2, cube2)
x1 = lam2
y1 = (a2 + amp_halpha*(1./math.sqrt(2.*math.pi))*np.exp((-((x1 - center_halpha)/sigma_halpha)**2.)/2.)/sigma_halpha
     + amp_nii1*(1./math.sqrt(2.*math.pi))*np.exp((-((x1 - center_nii1)/sigma_nii1)**2.)/2.)/sigma_nii1
     + amp_nii2*(1./math.sqrt(2.*math.pi))*np.exp((-((x1 - center_nii2)/sigma_nii2)**2.)/2.)/sigma_nii2)
plt.plot(x1,y1, color = 'red')

plt.subplot(3,1,3)
plt.plot(lam3,cube3)
x2 = lam3
y2 = (a3 + amp_oii1*(1./math.sqrt(2.*math.pi))*np.exp((-((x2 - center_oii1)/sigma_oii1)**2.)/2.)/sigma_oii1
     + amp_oii2*(1./math.sqrt(2.*math.pi))*np.exp((-((x2 - center_oii2)/sigma_oii2)**2.)/2.)/sigma_oii2)
